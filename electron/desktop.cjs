...
function menuTemplate(send, close, shortcuts = {}) {
  ...
  return [
    { label: '&File', submenu: [item('&New manuscript', 'new', 'Ctrl+N'), item('&Open…', 'open', 'Ctrl+O'), item('Recent manuscripts…', 'recent'), separator, item('&Save', 'save', 'Ctrl+S'), item('Save &as…', 'save-copy', 'Ctrl+Shift+S'), item('&Export…', 'export', 'Ctrl+Shift+E'), separator, item('Show in File Explorer', 'reveal'), separator, { label: 'E&xit', click: close }] },
    ...
    { label: '&View', submenu: [item('Focus mode', 'focus', 'F11'), item('Chapter outline', 'toggle-outline'), item('Writing assistant', 'toggle-assistant'), item('Continuous / divided pages', 'page-view'), separator, item('Revision history', 'history'), item('Create version checkpoint', 'snapshot'), separator, { role: 'resetZoom' }, { role: 'zoomIn' }, { role: 'zoomOut' }, separator, { role: 'togglefullscreen', accelerator: '' }] },
    { label: '&Settings', submenu: [item('Appearance and typography…', 'settings-appearance'), item('AI connections and task models…', 'settings-connections'), item('Local models…', 'settings-local'), item('Writing assistance…', 'settings-ai'), item('Language and translation…', 'settings-language'), item('Keyboard shortcuts…', 'settings-hotkeys')] }
  ];
}
