...
function menuTemplate(send, close, shortcuts = {}, recent = [], prefs = {}) {
  ...
  const numberingMenu = { label: 'Text numbering', submenu: ['Row', 'Page', 'Paragraph'].map(label => ({
    label: `${label} numbers`, type: 'checkbox', checked: Boolean(prefs[`show${label}Numbers`]), click: () => send(`toggle-${label.toLowerCase()}-numbers`)
  })) };
  const spellingMenu = { label: 'Spelling', submenu: [
    { label: 'Highlight spelling mistakes', type: 'checkbox', checked: prefs.spellcheck !== false, click: () => send('toggle-spellcheck') },
    item('Language and dictionaries…', 'settings-language')
  ] };
  ...
  return [
    ...
    { label: '&View', submenu: [item('Focus mode', 'focus', 'F11'), item('Chapter outline', 'toggle-outline'), item('Writing assistant', 'toggle-assistant'), item('Continuous / divided pages', 'page-view'), numberingMenu, separator, item('Next project tab', 'next-tab'), item('Previous project tab', 'previous-tab'), separator, item('Revision history', 'history'), item('Create version checkpoint', 'snapshot'), separator, { role: 'resetZoom' }, { role: 'zoomIn' }, { role: 'zoomOut' }, separator, { role: 'togglefullscreen', accelerator: '' }] },
    { label: '&Tools', submenu: [item('&Proofread…', 'proofread'), item('&Word count and statistics…', 'statistics'), separator, spellingMenu] },
    ...
    { label: '&Help', submenu: [item('Set up AI…', 'setup'), item('Writing tutorial…', 'tutorial'), item('About WRAITER', 'about')] }
  ];
}
