...
(async () => {
  ...
  try {
    ...
    await menu('View', 'Writing assistant'); const composer = page.getByRole('textbox', { name: 'Ask the writing assistant', exact: true });
    await composer.fill('Is the pacing clear?'); nextReply = JSON.stringify({ message: 'The pacing is clear.', done: true, tools: [] });
    ...
    const beforeChat = requests.length, longAnswer = `Background answer received.\n${'A useful detail for the author. '.repeat(180)}`; delay = 700; nextReply = JSON.stringify({ message: longAnswer, done: true, tools: [] });
    await composer.fill('First line'); await page.keyboard.press('Shift+Enter'); await page.keyboard.type('Second line');
    assert.equal(await composer.inputValue(), 'First line\nSecond line'); assert.equal(requests.length, beforeChat, 'Shift+Enter must add a line without sending');
    await page.keyboard.press('Enter'); await waitFor(() => requests.length > beforeChat, 'Enter sent chat request'); await page.locator('.chapter-select').filter({ hasText: 'Chapter one' }).click();
    await page.locator('.chat-message.assistant').filter({ hasText: 'Background answer received.' }).waitFor({ timeout: 12000 });
    const chatScroll = await page.locator('.inspector-body').evaluate(element => ({ bottom: element.scrollHeight - element.scrollTop - element.clientHeight, top: element.scrollTop, overflow: element.scrollHeight - element.clientHeight }));
    assert.ok(chatScroll.overflow > 0 && chatScroll.bottom <= 2, JSON.stringify(chatScroll)); delay = 0; await chapter2().click();
    const chatHistory = page.getByRole('combobox', { name: 'Previous chats', exact: true });
    assert.deepEqual(await chatHistory.locator('option').allTextContents(), ['Is the pacing clear?']);
    await page.getByRole('button', { name: 'New chat', exact: true }).click(); assert.equal(await page.locator('.chat-message').count(), 0);
    nextReply = JSON.stringify({ message: 'This is a separate conversation.', done: true, tools: [] }); await composer.fill('Discuss the ending separately'); await page.keyboard.press('Enter');
    await page.locator('.chat-message.assistant').filter({ hasText: 'This is a separate conversation.' }).waitFor();
    assert.deepEqual(await chatHistory.locator('option').allTextContents(), ['Discuss the ending separately', 'Is the pacing clear?']);
    await chatHistory.selectOption({ label: 'Is the pacing clear?' }); await page.locator('.chat-message.assistant').filter({ hasText: 'Background answer received.' }).waitFor();
    assert.equal(await page.locator('.chat-message').filter({ hasText: 'This is a separate conversation.' }).count(), 0);
    await waitFor(async () => (await recover()).chats?.length === 2, 'project chat history recovery');
    await waitFor(async () => JSON.parse(await fs.readFile(projectPath, 'utf8')).chats?.length === 2, 'project chat history named save');
    events.push('Chat uses Enter to send and Shift+Enter for a line break, follows replies to the bottom, survives chapter changes, and keeps multiple named conversations in the project.');
    ...
    assert.equal((await page.evaluate(() => window.wraiter.boot())).prefs.hotkeys.complete, 'Ctrl+Space');
    await menu('View', 'Writing assistant'); const restoredChats = page.getByRole('combobox', { name: 'Previous chats', exact: true });
    assert.deepEqual((await restoredChats.locator('option').allTextContents()).sort(), ['Discuss the ending separately', 'Is the pacing clear?'].sort());
    await restoredChats.selectOption({ label: 'Discuss the ending separately' }); await page.locator('.chat-message.assistant').filter({ hasText: 'This is a separate conversation.' }).waitFor();
    events.push('Relaunch restores manuscript, formatting, chapters, theme, custom shortcuts, chat sessions and their messages.');
