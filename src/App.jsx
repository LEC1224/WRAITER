...
export default function App() {
  ...
  return <div className={`app layout-${project.layout || 'story'} ${focus ? 'focus-mode' : ''}`} style={{ '--writing-font': docStyle.fontFamily, '--writing-size': `${docStyle.fontSize}pt`, '--writing-leading': docStyle.lineHeight, '--writing-measure': `${prefs.measure}px`, '--document-zoom': (prefs.zoom || 100) / 100 }}>
    ...
    {modal?.type === 'about' && <Modal title="WRAITER · 0.3.0" subtitle="Desktop writing with integrated AI assistance." onClose={() => setModal(null)}><p className="notice-copy">Open and save ODT, DOCX, Markdown, HTML and plain text directly. The writing assistant can edit your manuscript, and every text or formatting action has persistent undo and redo.</p><p className="notice-copy">Choose a writing layout and export the full manuscript, a chapter or selected text to office formats, PDF, EPUB, BBCode and more. Office documents with unsupported features need a compatibility review before overwriting; the complete original is preserved.</p><p className="small-muted">Live print pagination, comments, tracked changes, footnotes and direct Claude Code / Grok Build connections remain future work. Windows preview; macOS and Linux are not yet validated.</p></Modal>}
  </div>;
}
